...
Font_Klok = pygame.font.Font('Montserrat-Regular.ttf',28)
...
StandaardActiviteiten.append([0,"Volleybal",[1,4,3,0],[1,8,0,0],["Drinkbus"], (220,100,100),Sym_volley])
StandaardActiviteiten.append([1,"Voetballen",[1,0,3,0],[1,4,0,0],["Rakket","Drinkbus"],(220,100,100),Sym_voetbal])
StandaardActiviteiten.append([2,"Basket",[1,0,3,0],[1,4,0,0],["Rakket","Drinkbus"],(220,100,100),Sym_basket])
StandaardActiviteiten.append([3,"Andere",[1,6,0,0],[1,8,0,0],[],(220,150,180),Sym_andere])
...
ActiRect_Width = 0.16*screen.get_width()
ActiRect_Height = 0.4*ActiRect_Width
...
PlanBevestig_Knop = Button_Rechthoek(PlanRect.width/3,60,0, Bevestig_kleur,planningvlak,PlanRect.centerx+PlanRect.width/12,PlanRect.bottom-60,Font_KnopText,TekstKleur_licht,'Bevestigen')
PlanAnnuleer_Knop = Button_Rechthoek(PlanRect.width/3,60,0,Annuleer_kleur,planningvlak,PlanRect.left+PlanRect.width/12,PlanRect.bottom-60,Font_KnopText,TekstKleur_licht,'Annuleer')

Plan_Klok1 = Klok_Input(0.7*PlanRect2.width,0.6*PlanRect2_1.height,1/4,Font_Klok,planningvlak,TekstKleur_donker,PlanRect2_1.centerx-30,PlanRect2_1.centery)
Plan_Klok2 = Klok_Input(0.7*PlanRect2.width,0.6*PlanRect2_1.height,1/4,Font_Klok,planningvlak,TekstKleur_donker,PlanRect2_2.centerx-30,PlanRect2_2.centery)
...
HighlightRect1 = pygame.Rect(PlanRect2_1.left,PlanRect2_1.centery-HighlightRect_height/2,HighlightRect_width,HighlightRect_height)
...
while running:
    ...
    for surface in ( sleepvlak, bovenvlak, ondervlak,planningvlak,planRechthoeken, nieuweActivlak,deletevlak, keyboardvlak):
        surface.fill((10,10,10))

    #Achtergrond vullen
    screen.blit(Voetbalveld_img,(0,0))
    ...
    if BovenRechthoeken(ActiRect_Pos_x,ActiRect_Pos_y,ActiRect_Width,ActiRect_Height,Mouse,Mouse_Pos,Mouse_JustPressed,ActiRect_Slepen,ActiRect_Offset_x,ActiRect_Offset_y,sleepvlak,bovenvlak,ActiRect_StartPos_x,ActiRect_StartPos_Y,Font_Acti,StandaardActiviteiten,PlanningTrigger) and not PlanningTrigger and not DeleteMelding_Trigger:
        NieuweActi_Plannen =True


    #Nieuwe Activiteit plannen
    if NieuweActi_Plannen and not PlanningTrigger:
    ...
    if PlanningTrigger :
        if not PlanningLoop:
            PlanningLoop = True
            Plan_Klok1.Input_Tijd = PlanActiviteit[2].copy()
            Plan_Klok2.Input_Tijd = PlanActiviteit[3].copy()
            PlanBox1.Actief = [False]*len(PlanActiviteit[4])

        Planningsscherm_maken(overlay,planningvlak,PlanRect_Color,PlanRect,Font_PlanTitel,PlanRect_TitelColor,PlanRect1,Font_PlanKop1,PlanRect2,Font_PlanKop2,PlanRect2_1,
        ...
        if not Text_dict["PlanText_Active"]:
            ...
            if PlanBevestig_Knop.draw(1,Mouse,Mouse_Pos):   
                GeplandeActiviteiten.append([len(GeplandeActiviteiten), PlanActiviteit[1],Plan_Klok1.Input_Tijd,Plan_Klok2.Input_Tijd,ActieveBenodigdheden,DagRect_collision.index(True),PlanActiviteit[5]])
    ...
    if Agenda_lst[0] or DeleteMelding_Trigger:
...
#__________________________________________________________________________________________________________________________________________________________________________________
    ...
    for surface in (ondervlak,planRechthoeken, bovenvlak, sleepvlak,overlay,planningvlak,nieuweActivlak,deletevlak):
